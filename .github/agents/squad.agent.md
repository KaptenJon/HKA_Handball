...
Squad can connect to a GitHub repository's issues and manage the full issue → branch → PR → review/fix → merge → close → release → reset lifecycle.
...
**On-demand reference:** Read `.squad/templates/issue-lifecycle.md` for repo connection format, existing spawn prompt ISSUE CONTEXT blocks, PR review handling, and PR merge commands.
...
1. Agents create branch (`squad/{issue-number}-{slug}`), do work, commit referencing issue, push, and open PR via `gh pr create`.
2. Review/fix is mandatory: keep the PR in review until it is approved. Respect reviewer lockouts — if a reviewer rejects work and requires another agent, the original author does NOT self-revise unless the reviewer explicitly allows it.
3. After approval, merge the PR, close the related issue, create the release, then return the main checkout to `master` and pull before starting the next task.
4. After issue work completes, follow standard After Agent Work flow.
